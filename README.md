...
## Asynchronous request and synchronous request differences
Dengan <i>asynchronous request</i>, segala pekerjaan yang terjadi pada <i>client</i> akan diblokir. Pengguna tidak akan dapat melakukan apa-apa.

Sedangkan dengan <i>synchronous request</i>, pengguna tetap dapat melakukan pekerjaan seperti biasa dan <i>request</i> akan tetap dijalankan di balik layar.

## How does AJAX work?
Di Django, pengguna (<i>client</i>), akan mengirimkan sebuah <i>request</i>. Namun, <i>request</i> ini tidak akan langsung dikirimkan ke <i>server</i>, tetapi melewati AJAX terlebih dahulu.

Hal yang sama terjadi saat ada <i>response</i> yang diberikan oleh <i>server</i>. Akan melalui AJAX terlebih dahulu baru diteruskan ke pengguna atau <i>client</i>.

## Apa keuntungan AJAX? Dan bagaimana AJAX memengaruhi pengalaman pengguna pada sebuah website?
AJAX meningkatkan performa dan <i>response time</i>. Dengan adanya AJAX, pengguna juga dapat merasakan <i>experience</i> yang lebih baik karena situs yang lebih responsif, cepat, dan bahkan tidak perlu melakukan <i>reload</i> untuk mendapatkan data yang paling baru.

## Cara memastikan keamanan dengan AJAX saat login dan register
Hal ini dapat dilakukan dengan tetap menggunakan token CSRF walaupun harus menggunakannya secara manual. Karena ```{% csrf_token %}``` tidak dapat berjalan dengan baik dengan form AJAX, kita harus mengimplementasikannya secara manual. Dan menghindari penggunaan ```@csrf_exempt``` tanpa pertimbangan lebih lanjut.
